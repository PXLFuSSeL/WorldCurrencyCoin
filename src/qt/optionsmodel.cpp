...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeWCCAmount"))
        settings.setValue("nAnonymizeWCCAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeWCCAmount = settings.value("nAnonymizeWCCAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeWCCAmount"))
        SoftSetArg("-anonymizewccamount", settings.value("nAnonymizeWCCAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeWCCAmount:
            nAnonymizeWCCAmount = value.toInt();
            settings.setValue("nAnonymizeWCCAmount", nAnonymizeWCCAmount);
            emit anonymizeWCCAmountChanged(nAnonymizeWCCAmount);
            break;
        default:
            break;
        }
