...
/*
    if (IsProofOfStake() && !CheckCoinStakeTimestamp(nHeight, GetBlockTime(), (int64_t)vtx[1].nTime))
        return DoS(50, error("AcceptBlock() : coinstake timestamp violation nTimeBlock=%d nTimeTx=%u checkcoinstaketimestampe=%d IsProofOfStake=%d", GetBlockTime(), vtx[1].nTime, !CheckCoinStakeTimestamp(nHeight, GetBlockTime(), (int64_t)vtx[1].nTime), IsProofOfStake()));
*/
    // Check proof-of-work or proof-of-stake
    if (nBits != GetNextTargetRequired(pindexPrev, IsProofOfStake()))
        return DoS(100, error("AcceptBlock() : incorrect %s nBits=%d gntrReturn=%d", IsProofOfWork() ? "proof-of-work" : "proof-of-stake", nBits, GetNextTargetRequired(pindexPrev, IsProofOfStake())));

    // Check timestamp against prev
    if (GetBlockTime() <= pindexPrev->GetPastTimeLimit() || FutureDrift(GetBlockTime()) < pindexPrev->GetBlockTime())
        return error("AcceptBlock() : block's timestamp is too earlygetblocktime=%d GetPastTimeLimit=%d, FutureDrift=%d", GetBlockTime(), pindexPrev->GetPastTimeLimit(), FutureDrift(GetBlockTime()) < pindexPrev->GetBlockTime());
